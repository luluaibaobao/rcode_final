...
Bucket_position <- function(ContractID,ContractID_b, Date,Date_start,Date_end, Multiplier,Bucket,Para)
{
  ...
  Filename <- paste(Dir,"\\",paste(ContractID_b,Date_start,Date_end,sep = "-"),".binary",sep = "")
  load(Filename)
  Vol_border <- Detail$Vol_Max
  for(i in 1:Bucket)
  {
    Bucket_name[2*i-1] <- paste('l',i,sep = "")
    Bucket_name[2*i] <- paste('s',i,sep = "")
    Position[1,2*i-1] <-0
    Position[1,2*i] <-0
  }
  for(i in 1:Length)
  {
    ...
    {
      Position[i,2*Which_bucket-1] <- Data[i,]$VOL/Data[i,]$OPENINTS
      next
    }
    ...
    {
      Position[i,2*Which_bucket] <- Data[i,]$VOL/Data[i,]$OPENINTS
      next
    }
...
Bucket_position_cum<- function(ContractID, ContractID_b, Date, Date_start, Date_end, Multiplier,Bucket,Para)
{
  Data <- Bucket_position(ContractID, ContractID_b, Date, Date_start,Date_end, Multiplier,Bucket,Para)
  Position <- data.frame(c(1:length(Data[,1])))
  for(i in 1:(2 * Bucket))
  {
    Position[,i] <- Data[,(18+i)]
  }
  Position <- cumsum(Position)
  names(Position) <- names(Data[,19:(18+(2 * Bucket))])
  Filename <- paste(ContractID,"_",Date,".binary",sep = "")
  Position <- cbind(Data[,1:18],Position)
  save(Position, file = paste("C:\\data\\Position\\",Filename,sep = ""))
  return(Position)
}